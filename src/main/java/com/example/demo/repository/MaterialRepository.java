...
public interface MaterialRepository extends JpaRepository<Material,Long> {
    ...
    public List<Response> getJoinInformation();//query to getting material date about to finish
...
//    public List<Expire> getExpireStockInfor();
    ...
    @Query("SELECT new com.example.demo.dto.Expire(s.stock_id,s.stock_no,s.expire_date,s.quantity,m.mtr_name,m.mtr_bin) FROM Material m JOIN m.stocks s WHERE m.id=s.id  AND CURDATE() > s.expire_date AND s.quantity>0")
    public List<Expire> getExpireStockInfor();//query to getting expired products
