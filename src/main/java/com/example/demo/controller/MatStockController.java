...
public class MatStockController {
    ...
    @GetMapping("/getInfo")//get information about
    ...
    public ResponseEntity<Map<String,Boolean>>deleteStock(@PathVariable Long id){
        ...
        @GetMapping("/getExpireInfo")//get expire information for stock
    public List<Expire> getExpireStockInfor(){
        return materialRepo.getExpireStockInfor();
    }

    @DeleteMapping("/getExpireInfo/{stock_id}")//delete stocks by id
...
//    public ResponseEntity<Expire> getEXStocksById(@PathVariable Long stock_id){
//
//        Expire ex=expireRepo.findById(stock_id).orElseThrow(()->new ResourceNotFoundException("Stock not exists with id: "+stock_id));
